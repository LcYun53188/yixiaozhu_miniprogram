...
def build_doc():
    ...
    add_body_para(doc, "项目围绕“发布、审核、匹配、对接、反馈”构建完整业务闭环，既符合“AI 赋能·智创未来”的竞赛主题，也具备清晰的移动端落地价值。现阶段项目已经形成前端小程序、后端服务、数据库脚本和配套设计文档的完整骨架，具备继续完善并参与展示答辩的条件。")
    ...
    add_simple_table(
        ...
        ["功能模块", "功能说明", "比赛优先级"],
        [
            ["用户登录", "基于微信授权识别用户身份并区分普通用户与管理员角色", "高"],
            ["资源/需求发布", "支持发布“我有资源”与“我需要资源”两类信息，并填写标题、描述、图片、用户评估价、联系方式等", "高"],
            ["浏览与搜索", "支持按分类、关键词、时间和状态筛选信息", "高"],
            ["AI 辅助发布", "自动完成分类、标签提取、文案优化和基础风险提示", "高"],
    ...
    add_numbered(
        doc,
        [
            "用户通过微信授权登录系统，系统根据角色展示普通用户端或管理员端入口。",
            "普通用户在发布页填写资源或需求信息，并触发 AI 分类、标签提取和文案优化。",
    ...
    add_simple_table(
        ...
        ["核心数据表", "主要字段", "用途"],
        [
            ["user", "openid、nickname、role、phone、college", "存储普通用户、管理员和超级管理员信息"],
            ["resource", "title、description、category、tags、image_urls、estimated_price、review_status、flow_status", "存储“我有资源”记录"],
            ["need", "title、description、category、tags、estimated_price、urgency_level、review_status", "存储“我需要资源”记录"],
    ...
    add_body_para(doc, "其中，用户评估价字段用于表达发布者对物资或需求价值的主观参考判断，便于供需双方沟通和平台后续排序展示。该字段仅用于公益流转中的价值参考，不作为平台交易定价依据。")
    ...
    add_simple_table(
        ...
        [
            ...
            ["用户评估价", "发布资源或需求时可填写用户评估价，作为物资价值参考和沟通辅助信息", "帮助双方快速判断资源适配度和流转预期"],
            ["评价与信誉", "完成对接后支持星级评价、文字评价和信誉积分更新", "增强平台可信度并沉淀用户信用数据"],
            ["Docker 环境", "已提供 backend、mysql、phpMyAdmin 一体化开发环境", "降低联调门槛，提升演示稳定性"]
        ],
        [1.2, 3.5, 1.8],
    )
    add_body_para(doc, "其中，用户评估价并不代表平台鼓励交易，而是作为公益资源流转中的参考信息，帮助发布者更准确描述物资价值、帮助接收方判断资源适用性，也有助于后续在列表、详情和匹配结果中提供更清晰的信息展示。")
    add_body_para(doc, "如果后续切换到正式数据库存储，建议在 `resource` 与 `need` 表中增加 `estimated_price` 字段，类型可采用 `decimal(10,2)`，并在前端表单、详情页和后台审核页同步展示该字段，以保持项目文档、接口设计和系统实现的一致性。")
